...
async function deleteImageFromS3(fileKey: string) {
  try {
    ...
    console.log("Old image deleted from S3:", fileKey);
  } catch (err) {
...
export async function onSubmit(fileBuffer: Buffer, originalFilename: string, oldFileKey?: string) {
  try {
    ...
    const { url, fields } = await createPresignedPost(client, {
      Bucket: process.env.S3_AWS_BUCKET_NAME || "",
      Key: fileKey,
      Conditions: [["content-length-range", 0, 10485760], ["eq", "$Content-Type", contentType]],
      Fields: { "Content-Type": contentType },
      Expires: 60,
    });

    const formDataS3 = new FormData();
    Object.entries(fields).forEach(([key, value]) => formDataS3.append(key, value));
    formDataS3.append("file", new Blob([fileBuffer], { type: contentType }), originalFilename);

    const uploadResponse = await fetch(url, { method: "POST", body: formDataS3 });
    ...
    if (uploadResponse.ok) {
      ...
      console.error("Failed to upload new file:", errorMessage);
      return null;
    }
  } catch (err) {
